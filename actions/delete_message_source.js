...
/**
 * An action to delete message source in Real-time Insight service
 *
 ...
 * @param      {string}   name            (required)    Name of the message source
...
function main(params) {
    ...
    getMsgSources(baseUrl, authorizationHeader, params.name, function(err, res, body) {
        if (!err && res.statusCode === 200) {
            try {
                var parsedBody = JSON.parse(body);
                for (var schema in parsedBody) {
                    if (parsedBody[schema].name == params.name) {
                        deleteMsgSources(baseUrl, authorizationHeader, parsedBody[schema].id, function(err, res, body) {
            ...
            if (res) {
                console.error("Message Source can not be found (Status code: " + res.statusCode + ")");
                console.error(res.body);
                return whisk.error("Message Source can not be found (Status code: " + res.statusCode + ")");
            } else {
                console.error(err);
...
/**
 * A function to get all message sources in an RTI service instance.
 *
 ...
 */
function getMsgSources(baseUrl, authorizationHeader, name, callback) {
    var options = {
        method: 'GET',
...
function deleteMsgSources(baseUrl, authorizationHeader, id, callback) {
    var options = {
        method: 'DELETE',
        url: baseUrl + '/message/source/' + id,
        headers: {
            'Content-Type': 'application/json',
            'Authorization': authorizationHeader
        }
